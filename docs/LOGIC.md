...
### `quality_tag` / `quality_score` are placeholders

There is no trained page-quality model yet. Until there is, the stage writes a
grade derived entirely from `printed_or_handwritten`:
...
It therefore carries **no information** beyond `printed_or_handwritten`. Nothing
downstream branches on it, and review-ui deliberately does not surface it — the
confidence the UI shows is `hw_confidence`, the handwriting classifier's own
score. The single source of the mapping is `quality_placeholder()` in
`core-pipeline/db/__init__.py`; replacing the placeholder means deleting that
function and passing the model's output into `upsert_quality`.

In v7 this column held the handwriting classifier's *method* string, which is
why it never contained a quality grade. That value now lives in `hw_method`.
...
Tesseract reads handwriting badly. Judging a handwritten page on prelim text
would throw away real chart pages. So handwritten pages skip pass 1 entirely and
are judged in pass 2, on Azure Document Intelligence text.

```mermaid
flowchart TD
  subgraph P1["Pass 1 — prelim (Tesseract) text"]
    A{"handwritten?"} -- yes --> SKIP["skipped<br/>reason: handwritten"]
    A -- no --> CLS1["classify_text()"]
  end

  CLS1 --> OUT1{"verdict"}
  OUT1 -- "blank/junk/duplicate" --> DROP["No final OCR.<br/>Page is finished."]
  OUT1 -- "main" --> F["final OCR 1 + 2"]
  SKIP --> F

  subgraph P2["Pass 2 — final2 (Azure) text"]
...
**Stages:** `stages/ocr_final1_docling.py` (RapidOCR), `stages/ocr_final2_azure.py` (Azure Document Intelligence `prebuilt-read`)

Both run on the same page set: pages not ruled out by pass 1, **plus every
handwritten page** — those have no verdict yet, and their pass-2 verdict depends
on this text.

```mermaid
flowchart LR
  E{"eligible?"} -->|"handwritten"| YES["run"]
  E -->|"pass 1 said main"| YES
  E -->|"pass 1 said blank/junk/dup"| NO["skipped<br/>reason: blank_junk_pass1"]
  YES --> F1[("ocr_results<br/>'docling'")]
  YES --> F2[("ocr_results<br/>'azuredocintel'")]
```
...
Every consumer unwraps `content` — the review UI's Postgres adapter does it too.
...
The knock-on is real and documented in the log line: handwritten pages then get
no pass-2 verdict, and member/DOS fall back to final1/prelim text.
...
Behaviour above is pinned by `tests/` (108 tests). A failure there means the port
