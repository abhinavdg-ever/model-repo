...
> Last updated: 2026-09-18 (batch workers + chart_list pre-register; Azure retries; review-ui Docker :4000/:4001)
...
| 4 | `ocr_final1` | 1 | survivors + handwritten | `ocr_results` (docling) + `ocr/<chart>_final1.json` |
...
**Status: built (2026-09-18).** `POST /api/charts/batch` and `cli.py batch`
take `workers` (default `BATCH_WORKERS=4`). Charts are pre-registered into
`chart_list` as `received`, then run via `ThreadPoolExecutor`. Azure DI is
capped by `AZURE_DI_MAX_CONCURRENT`. The pool invariant
`workers × STAGE_WORKERS + headroom ≤ DB_POOL_MAX` is enforced at request time.

Decided 2026-09-14. Recorded here because it reverses a deliberate decision —
`run_batch` used to run charts one at a time, and the comment saying so was
load-bearing. Anyone changing the defaults should re-read the tradeoffs below.

### What is being asked for

`POST /api/charts/batch` gains `workers` (default `BATCH_WORKERS`, usually 4):
N charts run concurrently inside the API process. Pass `workers=1` for the
