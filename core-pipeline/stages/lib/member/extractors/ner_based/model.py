...
from .catalog import MODELS, by_id, model_dir, relink_local_paths
from .config import enabled_model_ids, ner_enabled
...
def predict_entities(
    ...
    if not snippet:
        return []
    if not ner_enabled:
        # The only behavioural difference from the reference. V1 always ran the
        # NER layer and raised on a bad model so a dead model could never look
        # like "nothing on the page". That contract is unchanged whenever the
        # layer is ON. With MEMBER_NER_ENABLED=false the caller has explicitly
        # asked for a rules-only run, and the engine stamps ner_enabled=false on
        # every row it writes, so the absence of hits stays visible.
        return []
